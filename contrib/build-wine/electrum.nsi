...
;Variables
  ...
  !define PRODUCT_NAME "Electrin"
  !define PRODUCT_WEB_SITE "https://github.com/takologi/electrin"
  !define PRODUCT_PUBLISHER "Takologi"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrin-setup.exe"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrin-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrin-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrin-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrin-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrin-${PRODUCT_VERSION}.exe" 0


  ;Links rincoin: and lightning: URIs to Electrin
  WriteRegStr HKCU "Software\Classes\rincoin" "" "URL:rincoin Protocol"
  WriteRegStr HKCU "Software\Classes\rincoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\rincoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\rincoin\shell\open\command" "" "$\"$INSTDIR\electrin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
  ...
  WriteRegStr HKCU "Software\Classes\lightning\shell\open\command" "" "$\"$INSTDIR\electrin-${PRODUCT_VERSION}.exe$\" $\"%1$\""
