...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrin-appimage-builder-img \
    "$CONTRIB_APPIMAGE"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    info "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrin_build/appimage/fresh_clone/electrin"
...
docker run $DOCKER_RUN_FLAGS \
    --name electrin-appimage-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrin \
    --rm \
    --workdir /opt/electrin/contrib/build-linux/appimage \
    electrin-appimage-builder-img \
    ./make_appimage.sh
