    ...
    function setSeedTypeHelpText() {
        var t = {
            'electrum': [
                ...
                qsTr('Electrin seeds are the default seed type.'),
                qsTr('If you are restoring from a seed previously created by Electrin, choose this option')
            ].join(' '),
            'bip39': [
                qsTr('BIP39 seeds can be imported in Electrin, so that users can access funds locked in other wallets.'),
                qsTr('BIP39 seeds do not include a version number, which compromises compatibility with future software.'),
            ].join(' '),
            'slip39': [
                qsTr('SLIP39 seeds can be imported in Electrin, so that users can access funds locked in other wallets.'),
            ].join(' ')
        }
