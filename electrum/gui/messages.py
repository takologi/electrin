...
It may be imported in another Electrin wallet with the same seed."""
)

MSG_LIGHTNING_WARNING = _(
"""Electrin uses static channel backups. If you lose your wallet file, you will need to request your channel to be force-closed by the remote peer in order to recover your funds. This assumes that the remote peer is reachable, and has not lost its own data."""
)
...
MSG_THIRD_PARTY_PLUGIN_WARNING = ' '.join([
    ...
    '<br/><br/>',
    _('Third-party plugins are not endorsed by Electrin.'),
    _('Electrin will not be responsible in case of theft, loss of funds or privacy that might result from third-party plugins.'),
    '<br/><br/>',
...
MSG_LN_EXPLAIN_SCB_BACKUPS = "".join([
    ...
    _("In the Electrin mobile app, use the 'Send' button to scan this QR code."), " ",
    "\n\n",
...
MSG_FREEZE_ADDRESS = _("When you freeze an address, the funds in that address will not be used for sending rincoins.")
MSG_FREEZE_COIN = _("When you freeze a coin, it will not be used for sending rincoins.")
...
MSG_TERMS_OF_USE = (
"""1. Electrin is distributed under the MIT licence by Electrin Technologies. Most notably, this means that the Electrin software is provided as is, and that it comes without warranty.

2. We are neither a bank nor a financial service provider. In addition, we do not store user account data, and we are not an intermediary in the interaction between our software and the Rincoin blockchain. Therefore, we do not have the possibility to freeze funds or to undo a fraudulent transaction.

3. We do not provide private user support. All issue resolutions are public, and take place on Github or public forums. If someone posing as 'Electrin support' proposes to help you via a private channel, this person is most likely an imposter trying to steal your rincoins."""
)
...
MSG_CONNECTMODE_SERVER_HELP = _(
    "Electrin connects to a unique server in order to receive your transaction history. "
    "This server will learn your wallet addresses."
)
MSG_CONNECTMODE_NODES_HELP = _(
    "In addition to your history server, Electrin will try to maintain connections with ~10 extra servers, in order to download block headers and find out the longest blockchain. "
...
MSG_CONNECTMODE_AUTOCONNECT_HELP = _(
    "Electrin will always use a history server that is on the longest blockchain. "
    "If your current server is unresponsive or lagging, Electrin will switch to another server."
)

MSG_CONNECTMODE_MANUAL_HELP = _(
    "Electrin will stay with the server you selected. It will warn you if your server is lagging."
)

MSG_CONNECTMODE_ONESERVER_HELP = _(
    "Electrin will stay with the server you selected, and it will not connect to additional nodes. "
